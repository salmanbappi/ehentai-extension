# ExHentai Extension (Mihon/Tachiyomi)
...
The extension registers a **single source** named **"ExHentai"** (in the *All* category). Instead of one source per language, pick a language from the **Language filter** in search — All, Japanese, English, Chinese, Dutch, French, German, Hungarian, Italian, Korean, Polish, Portuguese, Russian, Spanish, Thai, Vietnamese, N/A (Languageless) or Other — the same way Kototoro exposes `availableLocales` for its multilingual sources.
...
Your existing repo URL keeps working — ExHentai simply appears next to your other extensions:
...
- If you also have keiyoushi's E-Hentai installed, both can coexist — this extension registers a single source with its own ID.
- The single-source restructure changed the source ID — libraries from the older 17-per-language builds need re-adding once.
